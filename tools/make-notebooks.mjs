...
const SETUP = code(
  ...
  '    # Both streams: the suite writes warnings to stderr, and a helper that',
  '    # showed only stdout would hide exactly the diagnostics worth reading.',
  '    if r.stderr.strip():',
...
const nb01 = notebook([
  ...
  code(
    'upstream = write("lib/upstream.ttl", """',
  ...
  code(
    'r = suite("checks", "--ontology", ours, "--import-dir", WORK / "lib",',
    '          "--engine", "sparql",',
  ...
  md('### The near-miss that returns nothing',
    ...
    'slightly wrong and the filter matches nothing — which once produced a',
    'confidently empty report that looked exactly like a passing gate.',
    '',
    'It now warns, and names the namespaces it *did* see. Run it once deliberately',
    'so you recognise both the mistake and the message:'),
  code(
    ...
    'print("\\n^ zero findings -- but read the WARNING above the count.")'),
...
const nb02 = notebook([
  ...
  code(
    ...
    '# --advanced flag. The CLI is used through most of this notebook because it',
    '# shows the exit code, which is what a CI job actually branches on. The same',
    '# rules are reachable from Python (see the last section) from 0.1.8 onward.',
  ...
  md('---',
    ...
    '',
    'From **0.1.8** onward every binding reaches rules the same way — as an',
    '`inference` mode, which is what they are from a caller\'s point of view:',
    'triples in the report\'s world that are not in the input. The WebAssembly',
    'build takes the same argument, so this works in a browser too.',
    '',
    'An unrecognised mode is an **error**, never a silent fall back to `"none"` —',
    'which matters, because validating without applying the rules you asked for',
    'would report a conformance you never established.'),
  code(
    'if not HAVE_SHACL:',
    '    print("skipped: pip install shacl")',
    'else:',
    '    import shacl as shacl_py',
    '    s = shacl_py.Shapes.from_turtle(Path(WORK / "triple.ttl").read_text(encoding="utf-8"))',
    '    d = Path(WORK / "people.ttl").read_text(encoding="utf-8")',
    '    for mode in ("none", "rdfs", "rules", "rules-iterated", "bogus"):',
    '        try:',
    '            rep = s.validate_turtle(d, inference=mode)',
    '            print(f"  inference={mode:16} conforms={rep.conforms}  results={len(rep.results)}")',
    '        except Exception as e:',
    '            print(f"  inference={mode:16} ERROR: {str(e)[:60]}")'),
  ...
  md('---',
    ...
    '- The **engine** runs rules in every binding, including WebAssembly — but the',
    '  **VS Code extension** asks for `inference: "none"` deliberately, so rules in',
    '  your shapes do not fire in the editor. Expect the editor and CI to differ',
    '  when rules are in play.',
    '',
...
const nb03 = notebook([
  ...
  md('Look closely at what the diff does **not** contain: the comment above the',
    'query still says `acme:Engineer`, because a comment is prose *about* code and',
    'renaming inside it turns a true remark into a false one. Only the code line',
    'changed.',
    '',
    'It did not always work that way — on an older build the substitution ran across',
    'the whole file and left the comment reading *"every row is typed',
    '`acme:SoftwareEngineer`, which v2.0.0 renames to `acme:SoftwareEngineer`"*.',
    '',
    'String **literals** are still rewritten, deliberately. In a TARQL query an IRI',
    'template is built out of them — `CONCAT("acme:_Engineer_", ?id)` — so a term',
    'rename usually *must* reach inside a literal. That is a judgement about this',
    'domain rather than a general rule, which is why the fix is not simply',
    '"substitute outside comments and strings".',
    '',
    'The advice is unchanged: prefer the default dry-run in automation and review the',
    'patch, rather than letting an unattended job commit its own output.',
    '',
    '---',
    '',
    '## What to take away',
